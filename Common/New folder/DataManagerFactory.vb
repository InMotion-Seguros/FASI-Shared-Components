...
#Region "Execute Scalar Methods"
    ...
    Public Function QueryExecuteScalarToInteger() As Integer
        ...
        Dim parameters As New Dictionary(Of String, Object) From {{"CompanyId", _command.CompanyId}, {"Id", _id}}
        ...
        If Not ForceLocalMode Then
            If "DataManager.Mode".AppSettings() = "remote" Then
                local = False
            End If
        Else
            local = True
        ...
        If local Then
            With New Services.DataManager
                result = .QueryExecuteScalarToInteger(_command, parameters)
            End With
        Else
            Using dataService As DataManager.DataManagerClient = DataServiceInstance()
                result = dataService.QueryExecuteScalarToInteger(_command, parameters)
            End Using
        End If

        If "DataAccessLayer.Debug.Proxy".AppSettings(Of Boolean) AndAlso Not local Then
            watch.Stop()
            message = "              Mode: {1}{2}Id: {0}".SpecialFormater(_id, "DataManager.Mode".AppSettings(), Environment.NewLine) + Environment.NewLine + CommandSummary(_command)
            ...
            message &= Environment.NewLine + "              Scalar={0}".SpecialFormater(result)
...
#Region "SQL Statement Execute Methods"
    ...
    Public Function QueryExecuteToTable(resultEmpty As Boolean) As DataTable
        ...
        If ConfigurationManager.AppSettings("DataAccessLayer.Debug.Proxy").IsNotEmpty AndAlso
           ConfigurationManager.AppSettings("DataAccessLayer.Debug.Proxy").Equals("True", StringComparison.CurrentCultureIgnoreCase) Then
            watch = New Stopwatch
            watch.Start()
        End If
        ...
        Select Case Cache
            Case Enumerations.EnumCache.None
                If local Then
                    With New Common.Services.DataManager
                        QueryResult = .QueryExecuteToTable(_command, resultEmpty, New Dictionary(Of String, Object) From {{"CompanyId", _command.CompanyId}})
                    End With
                Else
                    Using dataService As DataManager.DataManagerClient = DataServiceInstance()
                        QueryResult = dataService.QueryExecuteToTable(_command, resultEmpty, New Dictionary(Of String, Object) From {{"CompanyId", _command.CompanyId}})
                    End Using
                End If
            Case Enumerations.EnumCache.CacheWithFullParameters
        ...
        If ConfigurationManager.AppSettings("DataAccessLayer.Debug.Proxy").IsNotEmpty AndAlso
           ConfigurationManager.AppSettings("DataAccessLayer.Debug.Proxy").Equals("True", StringComparison.CurrentCultureIgnoreCase) Then
            watch.Stop()
            message = CommandSummary(_command)
            message &= String.Format("              Time retrieve={0} ms", watch.ElapsedMilliseconds)
            ...
            End If
            message &= Environment.NewLine + String.Format("              Cache={0}", fromCache)
            If ConfigurationManager.AppSettings("DataAccessLayer.Debug.Proxy.Detail").IsNotEmpty AndAlso
               ConfigurationManager.AppSettings("DataAccessLayer.Debug.Proxy.Detail").Equals("True", StringComparison.CurrentCultureIgnoreCase) Then
                message &= Environment.NewLine + StackTraceSummary()
            End If
            InMotionGIT.Common.Helpers.LogHandler.TraceLog("DataAccessFactory", message)
        End If
