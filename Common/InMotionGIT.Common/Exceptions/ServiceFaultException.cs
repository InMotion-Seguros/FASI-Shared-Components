...
namespace InMotionGIT.Common.Exceptions
{
    ...
    public sealed class ServiceFaultException
    {
        ...
        public static FaultException Factory(Exception inner)
        {
            ...
            if (ConfigurationManager.AppSettings["Working.Mode"].IsNotEmpty() && ConfigurationManager.AppSettings["Working.Mode"].Equals("Development", StringComparison.CurrentCultureIgnoreCase))
            {
                IsDeveloperMode = true;
            }

            if (ConfigurationManager.AppSettings["FaultExceptionMessage"].IsNotEmpty())
            {
                ...
                case var @case when @case == typeof(DataAccessException):
                    {
                        ...
                        fault = new FaultException(message, new FaultCode(code, source));
                        break;
                    }
        ...
        public static FaultException Factory(string reason, Exception inner)
        {
            string source = Assembly.GetCallingAssembly().FullName;
            string code = DateTime.Now.ToString("yyyy.MM.dd.hh.mm.ss.fff");
            var fault = new FaultException(new FaultReason(reason), new FaultCode(code, source));
            fault.Source = source;
        ...
        public static FaultException Factory(string reason, string code)
        {
            ...
            var fault = new FaultException(new FaultReason(reason), new FaultCode(code, source));
            fault.Source = source;
            Helpers.LogHandler.ErrorLog(source, reason, null, string.Empty, false, code);
            return fault;
        }
        ...
        public static FaultException Factory(string reason)
        {
            string source = Assembly.GetCallingAssembly().FullName;
            string code = DateTime.Now.ToString("yyyy.MM.dd.hh.mm.ss.fff");
            var fault = new FaultException(reason, new FaultCode(code, source));
            fault.Source = source;
            Helpers.LogHandler.ErrorLog(source, reason, null, string.Empty, false, code);
            return fault;
        }
        ...
        public static FaultException Factory<TDetail>(TDetail container, Exception inner)
        {
            ...
            var fault = new FaultException<TDetail>(container, Message, new FaultCode(code, source));
            fault.Source = source;
        ...
        {
            string source = Assembly.GetCallingAssembly().FullName;
            var fault = new FaultException<TDetail>(container, new FaultReason(reason), new FaultCode(code, source));
            fault.Source = source;
            Helpers.LogHandler.ErrorLog(source, reason, inner, string.Empty, false, code);
            return fault;
        }
        ...
        public static FaultException Factory<TDetail>(TDetail container, string reason)
        {
            string source = Assembly.GetCallingAssembly().FullName;
            string code = DateTime.Now.ToString("yyyy.MM.dd.hh.mm.ss.fff");
            var fault = new FaultException<TDetail>(container, reason, new FaultCode(code, source));
            fault.Source = source;
            Helpers.LogHandler.ErrorLog(source, reason, null, string.Empty, false, code);
            return fault;
        }
